...
from model import build_family, reconstruct  # noqa: E402
...
def main():
    ...
    for rp in families:
        ...
        orig = E[E["process_id"].isin(pids)].drop(columns=["sector", "location"])
        ...
        ok = (
            (o["flow"].astype(str).values == r["flow"].astype(str).values).all()
            and (o["flow_name"].astype(str).values == r["flow_name"].astype(str).values).all()
            and np.allclose(o["amount"].astype(float), r["amount"].astype(float), equal_nan=True)
        )
        n_ok += ok
        n_fail += not ok
    ...
    loc_of = dict(zip(P["process_id"], P["location"]))
    sector_of_pid = dict(zip(P["process_id"], P["sector"]))
    rebuilt_all["location"] = rebuilt_all["process_id"].map(loc_of)
