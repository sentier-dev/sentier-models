...
Tested on two families (using the open-source CBC solver via `pulp`, all
...
| Natural gas, at production onshore | 50 | 0.0311 | 0.0434 | 0.735 | 0.705 | 0.720 |
...
actually uses, our method correctly identifies 71–77% of them from zero
...
structurally valid, mass-balanced recipe. On the natural-gas family the
method beats the naive baseline on both metrics at once (MAE 0.0311 vs.
0.0434, F1 0.72).
...
| Natural gas, at production onshore | global | 0.0311 | 0.735 | 0.705 | 0.720 |
| Natural gas, at production onshore | regional | 0.0308 | 0.735 | 0.705 | 0.720 |
...
elsewhere. On natural gas the effect is negligible either way, because most
slots there already fall back to the global mean (too few same-region
training points to trust regionally). This confirms the improvement helps
exactly where we designed it to help (genuine regional outliers) without
being a universal fix — the natural next refinement is anchoring on a
member's own historical trend once BAFU has real multi-year time series to
anchor on, rather than cross-sectional neighbors.
